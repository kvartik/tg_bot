...
from ..keyboards import ClubCb, TplCb, clubs_list, confirm_kb, template_actions, templates_list
...
async def routine_clubs(session: AsyncSession, db_user: User) -> list:
    """Клубы, где пользователь может править рутину: manager/senior — свои."""
    clubs = []
...
async def show_templates(call: CallbackQuery, session: AsyncSession, club_id: int) -> None:
    ...
    text = (
        ...
        "Задачи создаются каждому, кто на смене, в указанное время.\n"
        "Нажмите на шаблон, чтобы выключить или удалить."
    )
...
async def cb_tpl_add(call: CallbackQuery, callback_data: TplCb, state: FSMContext) -> None:
    ...
    await call.message.edit_text(
        "Новый шаблон в формате <code>ЧЧ:ММ Название</code>\n"
        "Например: <code>22:00 Отчёт по кассе</code>"
    )
    await call.answer()
...
async def msg_tpl_text(message: Message, session: AsyncSession, state: FSMContext) -> None:
    try:
        ...
        await message.answer("Не понял формат. Пример: <code>22:00 Отчёт по кассе</code>")
        return
    data = await state.get_data()
    await state.clear()
    await services.add_template(session, data["club_id"], time_hhmm, title)
    await message.answer(f"✅ Шаблон добавлен: {time_hhmm} — {title}\n/menu")
...
async def cb_tpl_open(call: CallbackQuery, callback_data: TplCb, session: AsyncSession) -> None:
    ...
    if tpl is None:
        ...
        return
    state_text = "включён" if tpl.is_active else "выключен"
    await call.message.edit_text(
...
async def cb_tpl_del(call: CallbackQuery, callback_data: TplCb, session: AsyncSession) -> None:
    ...
    if tpl is None:
        ...
        return
    await call.message.edit_text(
        f"Удалить шаблон <b>{tpl.time_hhmm} {tpl.title}</b>?",
